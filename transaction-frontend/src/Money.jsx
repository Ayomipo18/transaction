...
class Money extends React.Component {
    ...
    submitTransferForm = e => {
        ...
        fetch('https://transactions-apii.herokuapp.com/transfer', {
                method : 'POST',
                headers: {
    ...
    submitAuth = e => {
        ...
        if(this.state.signup) {
            fetch('https://transactions-apii.herokuapp.com/register', {
                method : 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            fetch('https://transactions-apii.herokuapp.com/login', {
                method : 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    getOtherUsers = () => {
        fetch('https://transactions-apii.herokuapp.com/users')
        .then(res => res.json())
        .then(data => {
    ...
    getCredits = () => {
        fetch('https://transactions-apii.herokuapp.com/userCredits', {
                method : 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
    ...
    getDebits = () => {
        fetch('https://transactions-apii.herokuapp.com/userDebits', {
                method : 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
    ...
    render() {
        if(!this.state.authenticated) {
            return (
                <div className='auth-page'>
                    <form onSubmit={this.submitAuth} className="auth-form">
                        <input onChange={this.onInputChange('username')} type="text" placeholder='Username'/>
            ...
            let {username, balance} = this.state.userData;
            return (
                <div className='dashboard'>
                    ...
                    <section className="transactions">
                        ...
                        <div className="transactionsList">
                            {this.state.seeReceived ? this.state.credits.reverse().map(credit => <SingleTransaction key = {credit._id} type='credit' name={credit.user.username} date={credit.time} amount={credit.amount}/>) : this.state.debits.reverse().map(debit => <SingleTransaction key={debit._id} type='debit' name={debit.beneficiary.username} date={debit.time} amount={debit.amount}/>)}
                        </div>
                    </section>
                    {this.state.sending && 
                                ...
                                <div style={{padding: '12px'}}>
                                    ...
                                    <div>
                                        ...
                                        <select defaultValue='disabled' onChange={this.onInputChange('beneficiary')} id="beneficiary">
                                            ...
                                            {this.state.otherUsers.map(user => <option key={user._id} value={user._id}>{user.username || 'null'}</option>)}
                                        </select>
